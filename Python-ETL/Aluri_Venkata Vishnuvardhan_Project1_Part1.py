...
class StockStatRecord(AbstractRecord):
    ...
    def __init__(self, symbol, company_name, exchange_country, exchange_rate, price, shares_outstanding, net_income,
                 market_value_usd=None, pe_ratio=None):
    ...
    def __str__(self):
        return '{s.__class__.__name__}({s.name}, {s.company_name}, {s.exchange_country}, $price={s.price:.2f}, {s.exchange_rate:.2f}, {s.shares_outstanding:.2f}, $NetIncome={s.net_income:.2f}, $Cap={s.market_value_usd:.2f}, P/E={s.pe_ratio:.2f})'.format(
            s=self)
...
class StocksCSVReader(AbstractCSVReader):
    ...
    # Static member
    required_column_names = ['ticker', 'exchange_country', 'company_name', 'price', 'exchange_rate',
        ...
        if all(colums_of_row.count(column) == 1 for column in self.required_column_names):
            try:
                if (len(row['ticker']) != 0):
                    ...
                    stockRecord = StockStatRecord(row['ticker'], row['company_name'], row['exchange_country'],
                                                  exchange_rate, price, shares_outstanding, net_income,
                                                  market_value_usd=market_value_usd, pe_ratio=pe_ratio)
                    return stockRecord
                else:
